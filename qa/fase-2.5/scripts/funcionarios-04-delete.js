// QA Fase 2.5.4 (funcionarios) - teste funcional de DELETE.
// PREPARADO, AINDA NÃO EXECUTADO (ver STATUS.md).
// Testa DELETE em funcionarios e depois verifica (com a conta admin) se o
// registro ainda existe, confirmando exclusão real ou bloqueio.
//
// Rodar:
//   SUPABASE_TEST_EMAIL=<e-mail do usuario> \
//   TARGET_FUNCIONARIO_ID=<uuid> node --env-file=qa/.env qa/fase-2.5/scripts/funcionarios-04-delete.js
//
// Esperado (matriz definida pelo usuário): proprietario / admin -> excluem.
//   usuario / sem_vinculo -> bloqueados.
...
async function main() {
  ...
  if (!email || !password || !targetId) {
    console.error('Defina SUPABASE_TEST_EMAIL, SUPABASE_TEST_PASSWORD e TARGET_FUNCIONARIO_ID.');
    process.exit(1);
  }
  ...
  console.log(`\n== DELETE funcionarios id=${targetId} ==`);
  const delResp = await fetch(`${SUPABASE_URL}/rest/v1/funcionarios?id=eq.${targetId}`, {
    method: 'DELETE',
    headers: {
  ...
  const checkResp = await fetch(`${SUPABASE_URL}/rest/v1/funcionarios?id=eq.${targetId}&select=id,nome`, {
