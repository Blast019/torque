// QA Fase 2.5.3 (fornecedores) - teste funcional de SELECT + isolamento.
// PREPARADO, AINDA NÃO EXECUTADO (ver STATUS.md).
// Faz login real via Supabase Auth e consulta fornecedores tanto na Empresa A
// quanto na Empresa B, validando a policy de SELECT e o isolamento entre empresas.
//
// Rodar (exemplo, papel admin):
//   SUPABASE_TEST_EMAIL=<e-mail do admin> node --env-file=qa/.env qa/fase-2.5/scripts/fornecedores-02-select.js
//
// Esperado (hipótese, a confirmar): proprietario / admin / usuario -> veem os fornecedores
//   da Empresa A, e 0 linhas da Empresa B. sem_vinculo -> 0 linhas em ambas.
...
async function main() {
  ...
  for (const [label, empresaId] of [['Empresa A', EMPRESA_A], ['Empresa B', EMPRESA_B]]) {
    const resp = await fetch(
      `${SUPABASE_URL}/rest/v1/fornecedores?empresa_id=eq.${empresaId}&select=*`,
      { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` } }
    );
    const respData = await resp.json();
    console.log(`\n== SELECT fornecedores - ${label} (HTTP ${resp.status}) ==`);
    if (resp.ok) {
