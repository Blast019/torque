// QA Fase 2.5.5 (movimentos_caixa) - teste funcional de INSERT tipo='entrada'.
// EXECUTADO E APROVADO EM 26/08/2026 (ver STATUS.md).
//
// Testa os 4 papéis de uma vez (script all-in-one, diferente do padrão de uma
// invocação por papel usado nas fases anteriores) porque a policy de INSERT aqui
// depende do valor da coluna `tipo`, e queremos comparar lado a lado o
// comportamento de INSERT entrada vs INSERT saida (script 03) para o mesmo papel.
//
// Policies confirmadas via SQL (25/08/2026):
//   caixa_insert_proprietario: WITH CHECK papel = 'proprietario' (qualquer tipo)
//   caixa_insert_despesa_admin_gerente: WITH CHECK tipo='saida' AND papel IN (admin,gerente)
// -> para tipo='entrada', só proprietario deveria conseguir. admin, usuario e
//    sem_vinculo devem ser bloqueados (403, RLS).
//
// Rodar: node --env-file=qa/.env qa/fase-2.5/scripts/movimentos-caixa-02-insert-entrada.js
  ...
  for (const papel of PAPEIS) {
    ...
    const insResp = await fetch(`${SUPABASE_URL}/rest/v1/movimentos_caixa`, {
      ...
      body: JSON.stringify({
        ...
        tipo: 'entrada',
        categoria: 'outro',
        descricao: `[TESTE] QA Fase 2.5.5 - entrada por ${papel.nome}`,
        valor: 100,
        data: hojeLocal(),
      }),
    });
    ...
    // Classificação depende só do HTTP status, não de uma linha no corpo - com
    // return=minimal o sucesso (201) não traz o registro criado. A confirmação
    // definitiva de que a linha existe de fato é feita à parte, via consulta
    // independente pelo proprietario (único papel com SELECT nesta tabela), mesmo
    // critério já adotado no script 03 e nos bloqueios de UPDATE/DELETE.
