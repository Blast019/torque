// QA Fase 2.5.1 (Ordens de Serviço) - teste funcional de UPDATE (USING + WITH CHECK).
// Faz login real via Supabase Auth e tenta atualizar uma OS específica,
// validando "os_update_usuario_operador".
//
...
// a partir de uma OS que já é da A).
//
// Rodar (exemplo):
//   SUPABASE_TEST_EMAIL=<e-mail do usuario> \
//   TARGET_OS_ID=<uuid> PATCH_JSON='{"descricao":"[TESTE] editado"}' node --env-file=qa/.env qa/fase-2.5/scripts/os-03-update.js
...
async function main() {
  ...
  if (!email || !password || !targetId || !patchJson) {
    console.error('Defina SUPABASE_TEST_EMAIL, SUPABASE_TEST_PASSWORD, TARGET_OS_ID e PATCH_JSON.');
    process.exit(1);
  }
  ...
  console.log(`\n== UPDATE ordens_servico id=${targetId} com patch=${patchJson} ==`);
  const updateResp = await fetch(`${SUPABASE_URL}/rest/v1/ordens_servico?id=eq.${targetId}`, {
    method: 'PATCH',
    headers: {
