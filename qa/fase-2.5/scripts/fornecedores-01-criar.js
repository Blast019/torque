// QA Fase 2.5.3 (fornecedores) - teste funcional de INSERT.
// PREPARADO, AINDA NÃO EXECUTADO (ver STATUS.md).
// Faz login real via Supabase Auth e tenta criar um fornecedor na Empresa A,
// validando a policy de INSERT de "fornecedores" (nome exato ainda não confirmado -
// STATUS.md registra só "completo, mesmo padrão" de ordens_servico; confirmar via
// pg_policies antes de rodar, ver seção 2.5.3 do STATUS.md).
//
// Payload igual ao que o app real envia (script.js, handler #salvarFornecedorBtn).
//
// Rodar (exemplo, papel proprietario):
//   SUPABASE_TEST_EMAIL=<e-mail do proprietario> node --env-file=qa/.env qa/fase-2.5/scripts/fornecedores-01-criar.js
//
// Esperado (hipótese, a confirmar): proprietario / admin / usuario -> sucesso (201).
...
async function main() {
  ...
  const nome = process.env.NOME || '[TESTE] QA Fase 2.5.3 - Fornecedor';
  ...
  console.log('\n== INSERT em fornecedores (Empresa A) ==');
  const insertResp = await fetch(`${SUPABASE_URL}/rest/v1/fornecedores`, {
    method: 'POST',
    headers: {
