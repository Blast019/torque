// QA Fase 2.5.2 (pecas/estoque) - teste funcional de DELETE.
// Testa DELETE em pecas e depois verifica (com a conta admin) se o
// registro ainda existe, confirmando exclusão real ou bloqueio.
// Valida "pecas_delete_usuario_gestor" (usuario_pode_excluir_empresa:
// proprietario/admin/gerente podem, usuario NÃO pode - mesmo padrão já adotado em
// ordens_servico, os_delete_usuario_gestor).
//
// Rodar:
//   SUPABASE_TEST_EMAIL=<e-mail do usuario> \
//   TARGET_PECA_ID=<uuid> node --env-file=qa/.env qa/fase-2.5/scripts/pecas-05-delete.js
...
async function main() {
  ...
  if (!email || !password || !targetId) {
    console.error('Defina SUPABASE_TEST_EMAIL, SUPABASE_TEST_PASSWORD e TARGET_PECA_ID.');
    process.exit(1);
  }
  ...
  console.log(`\n== DELETE pecas id=${targetId} ==`);
  const delResp = await fetch(`${SUPABASE_URL}/rest/v1/pecas?id=eq.${targetId}`, {
    method: 'DELETE',
    headers: {
  ...
  const checkResp = await fetch(`${SUPABASE_URL}/rest/v1/pecas?id=eq.${targetId}&select=id,nome`, {
