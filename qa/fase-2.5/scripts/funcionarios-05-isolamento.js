// QA Fase 2.5.4 (funcionarios) - teste dedicado de isolamento entre empresas.
// PREPARADO, AINDA NÃO EXECUTADO (ver STATUS.md).
// Login como admin da Empresa A e tenta:
//  1) SELECT nos funcionários da Empresa B (esperado: 0 linhas)
//  2) INSERT de um funcionário com empresa_id = Empresa B (esperado: bloqueado pelo WITH CHECK)
//
// Rodar: node --env-file=qa/.env qa/fase-2.5/scripts/funcionarios-05-isolamento.js  (usa a conta admin fixa, já conhecida)
...
async function main() {
  ...
  console.log('\n== 1) SELECT funcionarios da Empresa B (deve retornar 0 linhas) ==');
  const selResp = await fetch(`${SUPABASE_URL}/rest/v1/funcionarios?empresa_id=eq.${EMPRESA_B}&select=*`, {
  ...
  console.log('\n== 2) INSERT de funcionário com empresa_id = Empresa B (deve ser bloqueado pelo WITH CHECK) ==');
  const insResp = await fetch(`${SUPABASE_URL}/rest/v1/funcionarios`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      ...
      nome: '[TESTE] QA Fase 2.5.4 - isolamento (não deveria ser criado)',
      cargo: 'Mecânico',
  ...
  if (insResp.ok && Array.isArray(insData) && insData.length > 0) {
    console.log('\n🚨 FALHA DE ISOLAMENTO: admin da Empresa A conseguiu inserir funcionário na Empresa B!');
  } else {
