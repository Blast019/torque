// QA Fase 2.5.2 (pecas/estoque) - teste dedicado de isolamento entre empresas.
// Login como admin da Empresa A e tenta:
//  1) SELECT nas peças da Empresa B (esperado: 0 linhas)
//  2) INSERT de uma peça com empresa_id = Empresa B (esperado: bloqueado pelo WITH CHECK,
...
//
// Rodar: node --env-file=qa/.env qa/fase-2.5/scripts/pecas-06-isolamento.js  (usa a conta admin fixa, já conhecida)
...
async function main() {
  ...
  console.log('\n== 1) SELECT pecas da Empresa B (deve retornar 0 linhas) ==');
  const selResp = await fetch(`${SUPABASE_URL}/rest/v1/pecas?empresa_id=eq.${EMPRESA_B}&select=*`, {
  ...
  console.log('\n== 2) INSERT de peça com empresa_id = Empresa B (deve ser bloqueado pelo WITH CHECK) ==');
  const insResp = await fetch(`${SUPABASE_URL}/rest/v1/pecas`, {
    ...
    body: JSON.stringify({
      ...
      nome: '[TESTE] QA Fase 2.5.2 - isolamento (não deveria ser criado)',
      qtd: 1,
      estoque_minimo: 0,
      custo: 1,
      preco: 1,
  ...
  if (insResp.ok && Array.isArray(insData) && insData.length > 0) {
    console.log('\n🚨 FALHA DE ISOLAMENTO: admin da Empresa A conseguiu inserir peça na Empresa B!');
  } else {
