// QA Fase 2.5.2 (pecas/estoque) - teste funcional de SELECT + isolamento.
// Faz login real via Supabase Auth e consulta pecas tanto na Empresa A
// quanto na Empresa B, validando "pecas_select_usuario_vinculado"
// (usuario_pertence_empresa) e o isolamento entre empresas no mesmo teste.
//
// Rodar (exemplo, papel admin):
//   SUPABASE_TEST_EMAIL=<e-mail do admin> node --env-file=qa/.env qa/fase-2.5/scripts/pecas-03-select.js
//
// Esperado: proprietario / admin / usuario -> veem as peças da Empresa A, e 0 linhas da Empresa B.
...
async function main() {
  ...
  for (const [label, empresaId] of [['Empresa A', EMPRESA_A], ['Empresa B', EMPRESA_B]]) {
    const resp = await fetch(
      `${SUPABASE_URL}/rest/v1/pecas?empresa_id=eq.${empresaId}&select=*`,
      { headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${accessToken}` } }
    );
    const respData = await resp.json();
    console.log(`\n== SELECT pecas - ${label} (HTTP ${resp.status}) ==`);
    if (resp.ok) {
